...
function download(network: string, address: string, poolVersion: string) {
  if (network == 'mainnet' && !fs.existsSync(`etherscan/v2EthStableDebtAllTokens/${address}`)) {
    ...
    // @ts-ignore
    runCmd(
      `cast etherscan-source --chain-id ${network} -d ${downloadPath} ${address} --etherscan-api-key ${API_KEYS[network]}`
    );
  }
}
...
function downloadReference(address: string) {
  runCmd(
    `cast etherscan-source --chain-id mainnet -d etherscan/v2EthStableDebtToken ${address} --etherscan-api-key ${API_KEYS.mainnet}`
  );
}
...
function flatten(address: string, network: string, poolVersion: string) {
  ...
  if (address == '0xf37e202e587c6f63fd70f35c24eb7f818cc5d01a') {
    // v2 Rai stable debt token impl
    sourcePath = `etherscan/v2EthStableDebtAllTokens/${address}/StableDebtToken/StableDebtToken.sol`;
  } else if (address == '0x0e8f4fc4c261d454b13c74507bce8c38aa990361') {
    // v2 AMPL stable debt token impl
  ...
  } else if (address == '0x8180949ac41ef18e844ff8dafe604a195d86aea9') {
    // v2 stETH stable debt token impl
  ...
  } else if (address == '0x9c2114bf70774c36e9b8d6c790c9c14ff0d6799e') {
    // v2 oneInch stable debt token impl
    sourcePath = `etherscan/v2EthStableDebtAllTokens/${address}/StableDebtToken/src/stableDebt.sol`;
  } else if (address == '0xD23A44eB2db8AD0817c994D3533528C030279F7c') {
    // default impl
...
function getDiffsAgainstReference(fromContract: string, network: string, poolVersion: string) {
  ...
  if (network == 'mainnet') {
    before =
      'etherscan/v2EthStableDebtAllTokens/flattened/0xD23A44eB2db8AD0817c994D3533528C030279F7c.sol'; // default impl
    after = `etherscan/v2EthStableDebtAllTokens/flattened/${implAddress}.sol`;
    out = `v2EthStableDebtAll/${implAddress}`;
  } else if (network == 'polygon') {
    before =
      'etherscan/v2PolStableDebtAllTokens/flattened/0x72a053fa208eaafa53adb1a1ea6b4b2175b5735e.sol'; // default impl
